...
export default function Prac1() {
  ...
  // 이름을 작성하는 input에 onchange 속성으로 setInputUser를 사용해서  value로 업데이트됨
  // 해당 input의 value로 업데이트 됨
  const [inputUser, setInputUser] = useState("");

  //inputEmail stae는 사용자가 입력한 자신의 메일 주소를 저장하는 state
  //메일을 작성하는 input에 value로 inputEmail이 들어가 있음
  //메일을 작성하는 input에 onchange 속성으로 onCjangeEmail 함수 연결
  ...
  //그렇기 때문에 data.length -> data 배열의 길이에 1을 더해  직접 id값을 만듦
  ...
  // onChangeEmail 함수 설명
  //사용자의 email을 받는 input의 변경사항이 있을 때 실행되는 함수
  ...
  const eventClick = () => {
    ...
    // 그래서 각 state가 빈 문자열이 되면 해당 input도 빈 문자열로 바뀜
