...
//$() 선택자는 우리가 이전에 선택했던 방법과 다르게 jquery를 사용하기 위해 만들어진 함수이기 때문에 원래 document.querySelector 등으로 선택했던 요소를 다루는 방식으로 다룰 수 없음
//아래 코드 에러 발생
...
//------------------------------------------------
// html(): 요소의 content(html 형식) 가져오거나 수정
function useHtml() {
...
function useAppen() {
  ...
  //css 선택자를 자세하게 작성해서 특정 요소만 선택 할 수 있음
  $(".container .red:first-child").before(`<div class="box orange"></div>`);
  //after() : 선택된 요소의 뒤에 인자로 전달된 요소 추가
...
function useRemove() {
  ...
  //container라는 클래쓰를 가진 요소의 content(자식, 텍스트 모두)를 삭제
  $(".container").empty();
  //js는 empty 없음 ->innerHTML 속성을 한 문자열로 하면 동일하게 기능
...
function useParent() {
  ...
  //container 클래스를 가진 요소의 두 번째 자식이면서 box 자식이면서 box 클래스를 가진 요소를 선택
  //해당 요소의 이전 형제 요소 콘솔에 출력
  console.log('prev", $(".container .box:ntn-child(2)").prev()');
