...
//else문은 하나만 사용가능. 하지만 else if문은 여러개 사용 가능
...
if (score > 100) {
  ...
  //89~80점 사이의 점수 일 때 실행
  console.log("B");
} else if (score >= 70) {
  //79~70점 사이의 점수 일 때 실행
  console.log("c");
} else if (score >= 60) {
  // 69~60점 사이의 점수 일 떄 실행
  console.log("D");
} else {
  //59점 이하의 점수 일 때 실행
  console.log("F");
...
/**
 * JS DOC
 * 입력 받은 아이디와 비밀번호가 맞는지 확인 후 적절한 알림창 띄움
 *
 */
function loginuser() {
...
//   isloggedin === true ? nember > 10 ? "로그인한 유저인데, number 값이 10보다 큼" : "로그인한 유저인데, number 값이 10보다 작음 : 로그인하지 않는 유저 ;"
