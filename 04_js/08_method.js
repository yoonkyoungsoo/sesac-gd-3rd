...
//메서드는 자바스크립트에서 만들어 놓은 함수 0> 그래서 뒤에 괄호가 붙음
...
console.log(userID.length); //공백 짤리기 전이 문자열 길이 확인
...
//- rpeplace(교체될 값, 대신 들어갈 값): 교체될 값의 문자를 대신 들어갈 값으로 변환 -> 제일 처음에 만난 문자만 변환
//-replaveAll(): 해당하는 모든 문자를 교체
...
console.log(fruit.indexOf("mango")); //0
...
console.log(msg2.replace("Wow", "Ong")); //Ong! It's so amazing!!! Wow!!!!!!
console.log(msg2.replaceAll("Wow", "Ong")); //Ong! It's so amazing!!! Ong!!!!!!
console.log("msg2 값에 변화가 있는지 확인", msg2); // -> 변화 없음
...
let splitdate = date.split("."); // ['2024', '02', '28']
console.log(typeof splitdate);

//---------------------------------
//배열에서 사용 가능한 속성과 메서드
//push, pop, unshift, shift, indexOf, lenth
let arr1 = [1, 2, 3, 4, 5];
...
console.log("for of을 사용한 배열에서의 반복");
//for of
...
for (let alphabt of alphabts) {
  console.log(alphabt);
}

//forEach
//매개변수 2개를 받는 배열의 반복을 위한 메서드(=함수)
//매개변수 이름은 마음대로 지어도 상관 없지만 순서에 따르 의미는 항상 동일 -> 순서 중요!!!!
/**
...
//화살표 함수 예시
let newwArr3 = arr2.filter((animal) => animal.length >= 5);

//화살표 함수에서return까지 생략한 것
...
//find(): 배열에서 특정 조건을 만족하는 첫 번쨰 요소 반환
...
//배열의 모든 원소를 for문으로 돌려서 바꿀 수 있지만 더 간단히 메서드를 사용해 변경 가능
